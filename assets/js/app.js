...
function getMLB() {
    const url = 'https://api.mysportsfeeds.com/v2.0/pull/mlb/current/date/201808010/games.json';
    $.ajax({
        url: url,
        method: "GET",
        dataType: "json",
        ...
        success: function (response) {
            ...
            for (let i = 0; i < response.games.length; i++) {
                ...
                const scheduledTime = (moment(parsedTime).format("dddd, MMMM Do YYYY, h:mm:ss a"));
                // if (status === "UNPLAYED") {
                //     $("dt").removeClass("inning").addClass('not_played');
                //     $(".not_played").html(scheduledTime);
                // }    
                if ( status === "UNPLAYED"){
...
function inningStatus(status,scheduledTime) {
    if ( status === "COMPLETED") {
        $("dt").removeClass("inning").addClass("final");
        $(".final").html("FINAL");
    }
    else {
